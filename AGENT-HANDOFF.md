...
- 输入框前置模式包含 AI 生成 / 知识库问答 / 创建应用三类；选择知识库问答时，必须继续保留直接可见的知识库范围选择。
- 知识库范围提供「我的知识库 / 学校知识库」直接入口，默认选择学校知识库；不能把范围选择完全藏到 `+` 菜单里。
...
- 不要在问答模式隐藏知识库范围选择，只留下 `+` 菜单；用户已经指出老师点击知识库问答后仍需要继续限定范围。
- 不要把可见文案写回「学校共建库」。
...
- 【2026-06-08】当页面有吸底 CTA、底部 AI 输入栏、题目篮或右侧批注浮动按钮时，过稿切换控件必须做页面级避让：应用/资源详情页放左下安全区；知识库壳页避开左侧导航后放左下安全区；`ai-qbank.html` 放主内容左上，不能遮挡底部“去组题”、资源下载、一键改编或右侧批注按钮。

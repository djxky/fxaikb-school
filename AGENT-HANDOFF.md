...
- 【2026-07-15】学校管理页已从仅维护老师成员升级为学校基础数据入口：一级仅保留「学生与班级」「老师」。学生与班级按当前学年学期维护：学生可有一个行政班和多个教学班；班级成员关系、任教安排均带学年学期。课程不作为独立入口，改为「任教安排」中的学科：学年学期 × 班级 × 学科 × 老师 × 班级角色。老师可有主要学科与多个可授学科；可授学科不自动授予学生权限，只有当前有效任教安排才授予相应班级名单管理权限。任教老师（无论行政班或教学班）可添加、移出自己任教班级的学生；移出班级不改变学生在校状态。仅学校管理员可修改在读、休学、退学、毕业、转出、开除等状态，也可设置其他管理员和当前学年学期。进入新学年时按行政班自动升年级，教学班不自动延续，并先展示确认结果。名单导入支持 AI 识别、查重和错班提示，但只有确认后才写入；AI 不自动删除、合并学生或修改班级关系。
- 【2026-07-15】学校管理页视觉与信息架构：默认以普通任教老师身份进入「我的班级」，班主任与任课老师共用同一套班级名单工作台；管理员通过同一页面切换到全校范围，只额外看到新建班级、未分班学生、任教安排、老师和学年学期设置。不要为管理员另做一套首页。页面级不放“添加班级”或重复导入入口；新增学生、导入名单等动作必须落在已选定班级的上下文内。Demo 身份切换放在右上角账号菜单；真实页面按登录身份决定范围。视觉沿用暖白、深墨、低饱和金色，蓝色只可作为低强调反馈色。

...
(function(){
  ...
  function boot(){
    /* 03-wiki-entry.html 上的文件夹视图 → 用统一的 renderPersonalFolderView（v28.js 渲染）
       根据当前 active KB 切 scope，让数据/权限自动适配。
       团队 KB 默认 scope 用 active sb-kb 的 data-kb 拿；找不到就 fallback 到 math-group。 */
    if(typeof window.renderPersonalFolderView === 'function' && typeof window.setCurrentKbScope === 'function'){
      const activeKb = document.querySelector('.sb-kb.active')?.dataset?.kb;
      const fallbackTeam = ['math-group', 'school', 'grade-2', 'grade-3'].includes(activeKb)
        ? activeKb
        : 'math-group';
      window.setCurrentKbScope(fallbackTeam);
      window.renderPersonalFolderView();
      return;
    }
    /* 老路径兜底：v28.js 没加载（理论上不应发生） */
